#!/bin/bash
#SBATCH --job-name=leduc-both
#SBATCH --output=slurm_out/leduc-both-%j.out
#SBATCH --error=slurm_out/leduc-both-%j.out
#SBATCH --ntasks=1
...
# Build command
CMD="python -u paper_experiment_leduc_exploitability_comparison.py --run-both --n-workers $N_WORKERS --device-training cpu --device-parameter-server cpu --device-inference cpu"
