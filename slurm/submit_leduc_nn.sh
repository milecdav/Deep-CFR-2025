#!/bin/bash
#SBATCH --job-name=leduc-nn
#SBATCH --output=slurm_out/leduc-nn-%j.out
#SBATCH --error=slurm_out/leduc-nn-%j.out
#SBATCH --ntasks=1
...
# Build command
CMD="python -u paper_experiment_leduc_exploitability_comparison.py --run-nn --n-workers $N_WORKERS --device-training cpu --device-parameter-server cpu --device-inference cpu"
